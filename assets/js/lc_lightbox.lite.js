...
(function ($) {
	...
	var lcl = function(obj, settings) {
		...
		var lcl_vars = {
			...
			php_style		: '', // php tag style (for scrollbar hiding)
		...
		/* revert php entitles that might have been used in attrs (and trim) */
		var revert_php_entit = function(str) {
			if(!str) {return str;}
		...
		var attr_or_selector_data = function($elem, subj_key) {
			...
			if(subj.indexOf('> ') !== -1) {
				...
				return (typeof($elem.attr( subj )) != 'undefined') ? revert_php_entit( $elem.attr( subj )) : '';	
			}
		};
				...
				$.each(inst_obj, function(i,v) {
					...
					if(el_type && $.inArray(el_type, ['image']) !== -1) {
						if(typeof(v.src) != 'undefined' && v.src) {
							...
							el.title	= (typeof(v.title) == 'undefined') ? '' : revert_php_entit(v.title); 
							el.txt	 	= (typeof(v.txt) == 'undefined') ? '' : revert_php_entit(v.txt); 
							el.author 	= (typeof(v.author) == 'undefined') ? '' : revert_php_entit(v.author); 
		...
		var setup_code = function() {
			...
			// backup php/body inline CSS
			if(o.remove_scrollbar) {
				lcl_ai_vars.php_style = (typeof(jQuery('php').attr('style')) != 'undefined') ? jQuery('php').attr('style') : '';
			...
			// php is appended and ready - callback
			if(typeof(o.php_is_ready) == 'function') {
				o.php_is_ready.call(null, lcl_ai_opts, lcl_ai_vars);
			}
			
			// lightbox php has been appended and managed 
			if(!lcl_ai_vars.is_arr_instance) {
				var $subj = (lcl_ai_vars.elems_selector) ? $(lcl_ai_vars.elems_selector) : lcl_curr_obj;
				$subj.first().trigger('lcl_php_is_ready', [lcl_ai_opts, lcl_ai_vars]);
			}
		};
		...
		var side_to_over_txt = function(el, treshold, w, h, side_txt_w) {
			...
			if(w < treshold || (el.type != 'php' && h < treshold)) {
		...
		var switch_elem = function(new_el, slideshow_switch) {
			...
			setTimeout(function() {
				...
				// if switching from an php element - set static heights
				if(v.elems[v.elem_index].type == 'php') {
		...
		var close_lb = function() {
			...
			setTimeout(function() {
				...
				// restore php/body inline CSS
				if(lcl_ai_opts.remove_scrollbar) {
					jQuery('php').attr('style', lcl_ai_vars.php_style);
		...
		var setup_thumbs_nav = function() {
			...
			$.each(lcl_ai_vars.elems, function(i, v) {
				if(v.type != 'unknown') {
					...
					// if iframe and php and no poster - skip
					if((v.type == 'php' || v.type == 'iframe') && !bg) {return true;}
