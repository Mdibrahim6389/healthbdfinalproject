...
function before_and_after_replace(options) {
	...
	equal(simple.php(), expected, 'Inner php before and after replace equals.');
}
...
function before_and_after_remove(options) {
	...
	equal(simple.php(), one.php(), 'Inner php before and after remove one equals.');
	
	simple.trigger('remove.owl.carousel', [ 1 ]);
	simple.trigger('refresh.owl.carousel');
	
	equal(simple.php(), two.php(), 'Inner php before and after remove two equals.');
	
	simple.trigger('remove.owl.carousel', [ 0 ]);
	simple.trigger('refresh.owl.carousel');
	
	equal(simple.php(), all.php(), 'Inner php before and after remove all equals.');
}
...
function before_and_after_remove_add(options) {
	...
	equal(simple.php(), simpleClone.php(), 'Inner php before and after `remove()` and `add()` equals.');
}
...
test('destroy', function() {
	...
	var simple = $('#simple'),
		expected = simple.get(0).outerphp.replace(/\s{2,}/g, '');
	
	simple.owlCarousel().owlCarousel('destroy');
	
	equal(simple.get(0).outerphp.replace(/\s{2,}/g, ''), expected, 'Outer php before create and after destroy is equal.');
});
